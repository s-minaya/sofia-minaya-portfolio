...
`;function $({color:e=[1,1,1],speed:r=1,amplitude:i=.1,mouseReact:g=!0,className:b=""}){const p=u.useRef(null),c=u.useRef(null),a=u.useRef(null),l=u.useRef(null),v=u.useRef([.5,.5]);return u.useEffect(()=>{const t=p.current;if(!t)return;c.current||(c.current=new z({dpr:M()}));const m=c.current,n=m.gl;n.clearColor(1,1,1,1);const E=new N(n),f=new F(n,{vertex:U,fragment:j,uniforms:{uTime:{value:0},uColor:{value:new C(...e)},uResolution:{value:new C(1,1,1)},uMouse:{value:v.current},uAmplitude:{value:i},uSpeed:{value:r}}});a.current=f;const S=new L(n,{geometry:E,program:f}),d=()=>{const{offsetWidth:o,offsetHeight:s}=t;m.setSize(o,s),f.uniforms.uResolution.value.set(o,s,o/s)};d(),window.addEventListener("resize",d);const h=o=>{const s=t.getBoundingClientRect();v.current[0]=(o.clientX-s.left)/s.width,v.current[1]=1-(o.clientY-s.top)/s.height};g&&t.addEventListener("mousemove",h);const R=o=>{l.current=requestAnimationFrame(R),f.uniforms.uTime.value=o*.001,m.render({scene:S})};return l.current=requestAnimationFrame(R),t.contains(n.canvas)||t.appendChild(n.canvas),()=>{cancelAnimationFrame(l.current),window.removeEventListener("resize",d),g&&t.removeEventListener("mousemove",h),n.canvas.parentNode===t&&t.removeChild(n.canvas)}},[]),u.useEffect(()=>{a.current&&(a.current.uniforms.uColor.value.set(...e),a.current.uniforms.uAmplitude.value=i,a.current.uniforms.uSpeed.value=r)},[e,i,r]),T.jsx("div",{ref:p,className:`iridescence-container ${b}`})}export{$ as default};
