...
`,at=({particleCount:u=800,particleSpread:t=10,speed:e=.1,particleColors:s,moveParticlesOnHover:r=!1,particleHoverFactor:i=1,alphaParticles:m=!1,particleBaseSize:a=100,sizeRandomness:v=1,cameraDistance:x=20,disableRotation:M=!1,pixelRatio:j=1,className:k})=>{const E=C.useRef(null),P=C.useRef({x:0,y:0});return C.useEffect(()=>{const h=E.current;if(!h)return;const z=new X({dpr:j,depth:!1,alpha:!0}),c=z.gl;h.appendChild(c.canvas),c.clearColor(0,0,0,0);const S=new H(c,{fov:15});S.position.set(0,0,x);const R=()=>{const o=h.clientWidth,n=h.clientHeight;z.setSize(o,n),S.perspective({aspect:c.canvas.width/c.canvas.height})};window.addEventListener("resize",R,!1),R();const F=o=>{const n=h.getBoundingClientRect(),d=(o.clientX-n.left)/n.width*2-1,f=-((o.clientY-n.top)/n.height*2-1);P.current={x:d,y:f}};r&&h.addEventListener("mousemove",F);const g=u,I=new Float32Array(g*3),L=new Float32Array(g*4),_=new Float32Array(g*3),V=s&&s.length>0?s:tt;for(let o=0;o<g;o++){let n,d,f,b;do n=Math.random()*2-1,d=Math.random()*2-1,f=Math.random()*2-1,b=n*n+d*d+f*f;while(b>1||b===0);const A=Math.cbrt(Math.random());I.set([n*A,d*A,f*A],o*3),L.set([Math.random(),Math.random(),Math.random(),Math.random()],o*4);const G=et(V[Math.floor(Math.random()*V.length)]);_.set(G,o*3)}const q=new Y(c,{position:{size:3,data:I},random:{size:4,data:L},color:{size:3,data:_}}),B=new $(c,{vertex:st,fragment:rt,uniforms:{uTime:{value:0},uSpread:{value:t},uBaseSize:{value:a*j},uSizeRandomness:{value:v},uAlphaParticles:{value:m?1:0}},transparent:!0,depthTest:!1}),p=new J(c,{mode:c.POINTS,geometry:q,program:B});let T,O=performance.now(),w=0;const W=o=>{T=requestAnimationFrame(W);const n=o-O;O=o,w+=n*e,B.uniforms.uTime.value=w*.001,r?(p.position.x=-P.current.x*i,p.position.y=-P.current.y*i):(p.position.x=0,p.position.y=0),M||(p.rotation.x=Math.sin(w*2e-4)*.1,p.rotation.y=Math.cos(w*5e-4)*.15,p.rotation.z+=.01*e),z.render({scene:p,camera:S})};return T=requestAnimationFrame(W),()=>{window.removeEventListener("resize",R),r&&h.removeEventListener("mousemove",F),cancelAnimationFrame(T),h.contains(c.canvas)&&h.removeChild(c.canvas)}},[u,t,e,r,i,m,a,v,x,M,j]),N.jsx("div",{ref:E,className:`particles-container ${k}`})};export{at as default};
